...
-- SORTED BY
CREATE TABLE T_PART (A INT, B DECIMAL(10,2), C VARCHAR(255)) PARTITIONED BY (A) SORTED BY (A);

-- 创建分区表，以int类型列分区
CREATE TABLE T_PART_INT1 (A INT, B DECIMAL(10,2), C VARCHAR(255)) PARTITIONED BY (A);
-- 分批次插入数据
INSERT INTO T_PART_INT1 SELECT TINT, TDECIMAL, TVARCHAR FROM EX_PART_BUC WHERE TBIGINT BETWEEN 1000001 AND 1000005;
INSERT INTO T_PART_INT1 SELECT TINT, TDECIMAL, TVARCHAR FROM EX_PART_BUC WHERE TBIGINT BETWEEN 1000006 AND 10000010;
-- 查询表T_PART_INT
SELECT * FROM T_PART_INT1;
...
SHOW CREATE TABLE T_PART_PALLAS;
INSERT INTO T_PART_PALLAS SELECT TINT, TDECIMAL, TVARCHAR FROM EX_PART_BUC;
SELECT * FROM T_PART_PALLAS;
