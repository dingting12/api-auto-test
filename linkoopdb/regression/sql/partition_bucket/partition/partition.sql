...
DROP TABLE T_PART_BOOLEAN IF EXISTS;
...
DROP TABLE T_PART_H1000 IF EXISTS;
...
-- 创建外部表，10条数据
CREATE EXTERNAL TABLE EX_PART_BUC(TSMALLINT SMALLINT,TINT INT,TBIGINT BIGINT,TREAL REAL,TDOUBLE DOUBLE,TFLOAT FLOAT,TDECIMAL DECIMAL(10,2),TNUMERIC NUMERIC(10,4),TDATE DATE,TTIMESTAMP TIMESTAMP,TBOOL BOOLEAN,TCHAR CHAR(30),TVARCHAR VARCHAR(255))
location ('hdfs://node73:8020/node72/ruanna/file/partition_bucket.csv') FORMAT 'csv' (DELIMITER ',');
...
-- 创建分区表，以boolean类型列分区
CREATE TABLE T_PART_BOOLEAN (A BOOLEAN, B DECIMAL(10,2), C VARCHAR(255)) PARTITIONED BY (A);
-- 查看创建表的ddl
SHOW CREATE TABLE T_PART_BOOLEAN;
-- 查看系统表中该表的分区字段
select PERIPHERAL_INFO from information_schema.system_tables where TABLE_NAME = 'T_PART_BOOLEAN';
-- 插入数据，2个分区
INSERT INTO T_PART_BOOLEAN SELECT TBOOLEAN, TDECIMAL, TVARCHAR FROM EX_PART_BUC;
-- 查询表T_PART_BOOLEAN
SELECT * FROM T_PART_BOOLEAN;
...
LOCATION ('hdfs://node73:8020/node72/ruanna/test') FORMAT 'csv' (DELIMITER ',') PARTITIONED BY (TINT);
-- 查看创建表的ddl
