...
--创建表T_TYPE_VARCHAR1，设置字符长度为64000
...
--创建表T_TYPE_VARCHAR2，设置字符长度大于64000，是否报错
create table T_TYPE_VARCHAR2 (
name varchar(64001)
);

--创建表T_TYPE_VARCHAR3，设置为默认长度，默认长度为1
create table T_TYPE_VARCHAR3(
a1 varchar
);

--插入字符长度为1
insert into T_TYPE_VARCHAR3 values('a');
--查询表T_TYPE_VARCHAR3
select * from T_TYPE_VARCHAR3;

--插入字符长度>1，是否报错
insert into T_TYPE_VARCHAR3 values('ab');
--查询表T_TYPE_VARCHAR3
select * from T_TYPE_VARCHAR3;

--创建表T_TYPE_VARCHAR4，设置字符长度为30
create table T_TYPE_VARCHAR4(
a1 varchar(30)
);

--插入数据格式为双引号，报错
insert into T_TYPE_VARCHAR4 values("abcdef");
--查询表T_TYPE_VARCHAR4
