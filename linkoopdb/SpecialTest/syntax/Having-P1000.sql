...
SELECT TDATE,COUNT (*),SUM (TSMALLINT),MAX (TSMALLINT),MIN (TSMALLINT),AVG (TSMALLINT) FROM P1000 GROUP BY TINTEGER HAVING AVG (TSMALLINT) < 200 AND TDATE > TO_DATE('2019-09-11','YYYY-MM-DD') LIMIT 10;
-- 把查询出的数据insert到T_TYPE_HAV3表中
DROP TABLE T_TYPE_HAV3 IF EXISTS;
CREATE TABLE T_TYPE_HAV3 AS (SELECT TDATE,COUNT (*),SUM (TSMALLINT),MAX (TSMALLINT),MIN (TSMALLINT),AVG (TSMALLINT) FROM P1000 GROUP BY TINTEGER HAVING AVG (TSMALLINT) < 200 AND TDATE > TO_DATE('2019-09-11','YYYY-MM-DD')) WITH DATA;
-- 查看返回结果
