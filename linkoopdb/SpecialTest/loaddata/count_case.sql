...
-- 单个去重字段，统计SMALLINT类型字段无重复的条数，条数为944
select count(DISTINCT TSMALLINT ) from P1000_CY_COUNT;

-- 单个去重字段，统计INTEGER类型字段无重复的条数，条数为9998
...
-- 单个去重字段，统计TIMESTAMP类型字段无重复的条数，条数为9977491
select count(DISTINCT TTIMESTAMP) from P1000_CY_COUNT;

-- 单个去重字段，统计BOOL类型字段无重复的条数，条数为2
...
-- 多个去重字段，统计SMALLINT、DOUBLE、BOOLEAN和VARCHAR类型字段无重复的条数，条数为9999936
select count(DISTINCT TSMALLINT,TDOUBLE,TBOOL,TVARCHAR) from P1000_CY_COUNT;

-- 多个去重字段，统计INTEGER、DECIMAL、BOOLEAN和CHAR类型字段无重复的条数，条数为9999993
